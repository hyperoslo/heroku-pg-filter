...
function * list (context, heroku) {
  ...
  function filterDate (transfers) {
    if (context.flags.from) {
      const dateFrom = Date.parse(context.flags.from);
      return transfers.filter(function (el) {
        const createdAt = Date.parse(el.created_at);
        return dateFrom <= createdAt;
      });
    } else {
      const dateTo = Date.parse(context.flags.to);
      return transfers.filter(function (el) {
        const createdAt = Date.parse(el.created_at);
  ...
  if (context.flags.from || context.flags.to) {
    console.log("here");
    transfers = filterDate(transfers);
  }
...
module.exports = {
  topic: 'pg-filter',
  command: 'backups',
  description: 'list database backups adds filtering',
  needsApp: true,
  needsAuth: true,
  variableArgs: true,
  flags: [
    ...
    {name: 'from', char: 'f', hasValue: true, description: 'from date'},
    {name: 'to', char: 't', hasValue: true, description: 'to date'}
  ],
